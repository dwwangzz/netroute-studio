...
public sealed partial class ControlledCommandService : IControlledCommandService
{
    ...
    public async Task<ControlledCommandResult> ExecuteAsync(string input, CancellationToken cancellationToken = default)
    {
        ...
        return new ControlledCommandResult(command.DisplayCommand, started, DateTimeOffset.Now - started,
            timedOut || cancelled ? -1 : process.ExitCode, await outputTask, await errorTask, timedOut, cancelled);
    }

    private static ProcessStartInfo CreateStartInfo(ControlledCommand command)
    {
        var info = new ProcessStartInfo { FileName = command.Executable, UseShellExecute = false, CreateNoWindow = true, RedirectStandardOutput = true, RedirectStandardError = true, StandardOutputEncoding = Encoding.Default, StandardErrorEncoding = Encoding.Default };
